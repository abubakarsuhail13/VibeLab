...
app.get('/api/phases', authenticateToken, async (req: any, res) => {
  try {
    const p = await getPool();
    if (!p) return res.status(503).json({ error: 'Database connection failed' });
    ...
    // Fetch user progress for these phases
...
app.get('/api/phase/:id', authenticateToken, async (req: any, res) => {
  const { id } = req.params;
  try {
    const p = await getPool();
    if (!p) return res.status(503).json({ error: 'Database connection failed' });
...
app.get('/api/phase/:id/projects', authenticateToken, async (req: any, res) => {
  const { id } = req.params;
  try {
    const p = await getPool();
    if (!p) return res.status(503).json({ error: 'Database connection failed' });
    ...
    const [progress]: any = await p.execute(
      'SELECT project_id, completed_steps, is_completed FROM user_project_progress WHERE user_id = ?',
      [req.user.userId]
    );
...
app.post('/api/progress/update', authenticateToken, async (req: any, res) => {
  ...
  try {
    ...
    if (project.length > 0) {
      ...
      // Get all projects in this phase
