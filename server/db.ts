...
export const getPool = async () => {
  if (!pool) {
    ...
    try {
      ...
      if (isInitialized) {
        console.log('DB Debug: Database is already initialized. Standard verification and migration will proceed...');
      }
      ...
      try {
        ...
        try {
          ...
          const [phasesCountRes]: any = await connection.execute('SELECT COUNT(*) as count FROM phases');
          const phasesCount = phasesCountRes[0].count;
          ...
          if (phasesCount !== 5 || p2Check[0].count === 0) {
            console.log('DB Migration: Rebuilding schema to match the approved 5-phase structure...');
            
            // Delete old relationships first via cascade or manual purge
            await connection.execute(`
        ...
        // Add intro_completed columns in unified stream if they are not there
        try {
          ...
          console.warn("Could not migrate intro_completed columns in unified stream:", colErr.message);
        }
        ...
        // Seed 10 Quiz Questions per Phase dynamically if empty for that phase
        console.log('Verifying quiz questions seeding per phase...');
        const questionsByPhaseOrder: Record<number, any[]> = {
          ...
          4: [
            ...
            { q: 'What is the advantage of containerizing AI pipelines?', o: ['It ensures absolute dependency parity (CUDA, Python versions, packages) across any hosts', 'It replaces the need for database engines', 'It automatically writes documentation'], c: 1 },
        ...
        for (const [phaseOrderText, list] of Object.entries(questionsByPhaseOrder)) {
          ...
          if (pRow.length > 0) {
            ...
            const [qCount]: any = await connection.execute('SELECT COUNT(*) as count FROM quiz_questions WHERE phase_id = ?', [phaseId]);
            if (qCount[0].count === 0) {
        ...
        console.log('Verifying learning resources seeding per phase...');
        ...
        for (const [phaseOrderText, list] of Object.entries(resourcesList)) {
          ...
          if (pRow.length > 0) {
            const phaseId = pRow[0].id;
            const [rCount]: any = await connection.execute('SELECT COUNT(*) as count FROM phase_resources WHERE phase_id = ?', [phaseId]);
            if (rCount[0].count === 0) {
              for (const item of list) {
                await connection.execute(
